...
function enableButtons () {
	...
	// Update the button label now that the button is active
	btn.value="Click me for a p greeting";
	
	// Set the onclick action for the second button
	btn = document.getElementById("prediction");
	btn.onclick=function(){prediction();};
	
	// Update the button label now that the button is active
	btn.value="Click me to get prediction";
}
...
function prediction () {
	// Construct the request for the sayHello() function
	var request = gapi.client.helloworldendpoints.prediction();
